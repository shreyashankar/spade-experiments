...
async def generate_concepts_and_assertions(
    ...
    # Generate concepts
    try:
        response = await acompletion(model="azure/gpt-4-2", messages=messages)
...
async def generate_candidate_assertions(
    ...
    for prev_template, curr_template in zip(
        ...
        # Format curr template on example variables
        curr_formatted_prompt = curr_template.format(**example_variables)

        # Query GPT-3 with curr template
        messages = [
            {
                "content": curr_formatted_prompt,
                "role": "user",
            }
        ]
        response = litellm.completion(model="azure/gpt-35-turbo-16k", messages=messages)
        example_llm_response = response["choices"][0]["message"]["content"]

        # Gen assertions
        print("Generating assertions...")
        concepts_and_assertions = await generate_concepts_and_assertions(
            curr_template, prev_template, curr_formatted_prompt, example_llm_response
        )
